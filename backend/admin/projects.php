...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add') {
    ...
    // 3. Generate Slug and Database Insertion
    if (!empty($title) && !empty($category) && !empty($location) && !empty($description)) {
        $slug = generateUniqueSlug($pdo, $title);
        $stmt = $pdo->prepare("INSERT INTO projects (title, slug, category, location, description, image_url, status) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$title, $slug, $category, $location, $description, $image_path_for_db, $status]);
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-xl shadow border border-gray-100 overflow-hidden flex flex-col hover:shadow-lg transition-shadow">
                    ...
                    <div class="p-6 flex-1 flex flex-col">
                        <div class="text-xs text-gray-400 mb-1">Slug: <?php echo htmlspecialchars($proj['slug'] ?? 'N/A'); ?></div>
