 ...
 * Example: TXN20260323153045AB12
 */
$transaction_id = "TXN" . date("YmdHis") . strtoupper(substr(md5(uniqid(mt_rand(), true)), 0, 4));
...
/* PAN validation if 80G requested */
if ($wants_80g) {
    if (empty($pan_number)) {
        echo json_encode([
            "success" => false,
            "message" => "PAN number is required for 80G."
        ]);
        exit();
    }
...
/* Email unique check */
$checkStmt = $pdo->prepare("SELECT id FROM donations WHERE email = :email LIMIT 1");
$checkStmt->execute([":email" => $email]);

if ($checkStmt->fetch()) {
    echo json_encode([
        "success" => false,
        "message" => "This email is already used for a donation."
    ]);
    exit();
}
